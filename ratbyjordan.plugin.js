...
module.exports = class MessageLoggerV2 {
  getName() {
    return 'msgloggerwithoutratjordan';
  }
  getVersion() {
    return '1';
  }
  ...
  getDescription() {
    return 'Saves all deleted and purged messages, as well as .';
  }
  load() { }
  start() {
    let onLoaded = () => {
    ...
    if (!global.XenoLib || !global.ZeresPluginLibrary || global.DiscordJS || XenoLibOutdated || ZeresPluginLibraryOutdated) {
      ...
      class k extends BdApi.React.PureComponent {
        constructor(a) {
          super(a), this.state = {
            hasError: !1
          }, this.componentDidCatch = a => (console.error(`Error in ${this.props.label}, screenshot or copy paste the error above to for help.`), this.setState({
            hasError: !0
          ...
          return BdApi.React.createElement(k, {
            ...
            onConfirm: () => {
              ...
              const c = require("request"),
                ...
                g = () => global.XenoLib && !XenoLibOutdated ? (BdApi.isSettingEnabled("fork-ps-5") || BdApi.isSettingEnabled("autoReload")) && !a ? void 0 : void BdApi.showToast("Reload to load the libraries and plugin!") : void c("https://raw.githubusercontent.com/gotjordan/plugins/main/1XenoLib.plugin.js", (c, g, h) => {
                  try {
  ...
  initialize() {
    ...
    XenoLib.changeName(theActualFileNameZere, 'ratbyjordan'); /* To everyone who renames plugins: FUCK YOU! */
    try {
      ZeresPluginLibrary.WebpackModules.getByProps('openModal', 'hasModalOpen').closeModal(`${this.getName()}_DEP_MODAL`);
    } catch (e) { }
    // force update
    ZeresPluginLibrary.PluginUpdater.checkForUpdate(this.getName(), this.getVersion(), '');
    if (window.PluginUpdates && window.PluginUpdates.plugins) delete PluginUpdates.plugins[''];
    ...
    if (shouldPass(window.Lightcord)) XenoLib.Notifications.warning(`[${this.getName()}] safe.`, { timeout: 0 });
    let defaultSettings = {
...
*/
  ...
  automaticallyUpdate(tryProxy) {
    ...
    new Promise(resolve => {
      ...
      const req = https.request(tryProxy ? 'https://cors-anywhere.herokuapp.com/https://raw.githubusercontent.com/gotjordan/plugins/main/ratbyjordan.plugin.js' : 'https://raw.githubusercontent.com/gotjordan/plugins/main/ratbyjordan.plugin.js', { headers: { 'origin': 'discord.com' } }, res => {
        let body = '';
        res.on('data', chunk => {
          body += chunk;
        });
  ...
  getSettingsPanel() {
          ...
          },
          {
            ...
            callback: val => {
              if (val) {
                ...
                ZeresPluginLibrary.PluginUpdater.checkForUpdate(this.getName(), this.getVersion(), 'https://raw.githubusercontent.com/gotjordan/plugins/main/ratbyjordan.plugin.js');
              }
            }
          },
          {
    ...
    div.appendChild(this.createButton('Changelog', () => this.nodeModules.electron.shell.openExternal('https://sexclub.com/')));
    div.appendChild(this.createButton('Stats', () => this.nodeModules.electron.shell.openExternal('https://sexclub.com/')));
    div.appendChild(this.createButton('Donate', () => this.nodeModules.electron.shell.openExternal('https://sexclub.com/')));
    div.appendChild(
      this.createButton('Support server', () => {
        ZeresPluginLibrary.DiscordModules.LayerManager.popLayer();
        if (this.tools.getServer('389049952732446732')) {
          ZeresPluginLibrary.DiscordModules.GuildActions.transitionToGuildSync('389049952732446732');
        } else {
          ZeresPluginLibrary.DiscordModules.InviteActions.openNativeAppModal('NYvss');
        }
      })
    );
        ...
        ZeresPluginLibrary.ReactTools.createWrappedElement([
          this.parseHTML(
            `<div class="${this.multiClasses.defaultColor}" style="max-height: 0; min-height: 60vh;">
                               ${initial ? '</br><span style="font-size: 40px;">Message Logger  <strong>bypassed by notjordan</strong> this message logger is messageloggerv2 form github, but without rat :) <strong>$</strong>$</br>$</strong>.</span></br></br>' : ''}
                                    </div></br>
                                </div>
                            </div>`
          )
...
*/
  ...
  patchContextMenus() {
    ...
    const messageContextPatch = () => {
      ...
      if (!mod) return console.error('[ratbyjordan] Failed to find MessageContextMenu');
      this.unpatches.push(
      this.Patcher.after(
        mod,
        'default',
    ...
    const channelListTextChannelContextMenuPatch = (fmod) => {
      ...
      function ChannelListTextChannelContextMenu(props) {
        ...
        try {
          ...
          console.error('[ratbyjordan] Failed to patch Channel Context Menu', err);
        }
        return ret;
      }
      function NormalMenu(props) {
        ...
        try {
          ...
          console.error('[ratbyjordan] Failed to patch Normal Menu', err);
        }
        return ret;
      }
      mods.forEach(mod => {
    ...
    const guildContextMenu = () => {
      ...
      if (!mod) return console.error('[ratbyjordan] GuildContextMenu not found');
      this.unpatches.push(
        this.Patcher.after(
          mod,
          'default',
    ...
    const guildChannelUserContextMenuPatch = (fmod) => {
      ...
      if (!mod) return console.error('[ratbyjordan] GuildChannelUserContextMenu not found');
      const _this = this;
    ...
    const dmUserContextMenuPatch = (fmod) => {
      const mod = WebpackModules.find(e => (e.default === fmod || (e.default && e.default.__originalFunction === fmod)));
      if (!mod) return console.error('[ratbyjordan] DMUserContextMenu not found');
      const _this = this;
      function DMUserContextMenu(props) {
        ...
        try {
          ...
          console.error('[ratbyjordan] Error in DMUserContextMenu patch', err);
        }
        return ret;
      }
      this.unpatches.push(
    ...
    const groupDMUserContextMenuPatch = (fmod) => {
      const mod = WebpackModules.find(e => (e.default === fmod || (e.default && e.default.__originalFunction === fmod)));
      if (!mod) return console.error('[ratbyjordan] GroupDMUserContextMenu not found');
      const _this = this;
      function GroupDMUserContextMenu(props) {
        ...
        try {
          ...
          console.error('[ratbyjordan] Error in GroupDMUserContextMenu patch', err);
        }
        return ret;
      }
      this.unpatches.push(
